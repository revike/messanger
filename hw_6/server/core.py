...
class MassageProcessor(Thread):
    ...
    def authorize_user(self, message, sock):
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            ...
            hash_get = new(self.database.get_hash(message[USER][ACCOUNT_NAME]), random_str, 'MD5')
            ...
            if RESPONSE in ans and ans[RESPONSE] == 511 and compare_digest(digest, client_digest):
                ...
                self.database.user_login(message[USER][ACCOUNT_NAME], ip, port, message[USER][PUBLIC_KEY])
            else:
                response = RESPONSE_400
