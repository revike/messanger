...
from sqlalchemy import Column, Integer, String, DateTime, ForeignKey, create_engine, MetaData
...
class ServerDB:
    ...
    def user_login(self, name, ip, port):
        result = self.session.query(self.AllUsers).filter_by(login=name)
        if result.count():
            user = result.first()
    ...
    def login_history(self, name=None):
        ...
        if name:
            query = query.filter(self.AllUsers.login == name)
        return query.all()
...
if __name__ == '__main__':
    ...
    # print(db.login_history('client_2'))
